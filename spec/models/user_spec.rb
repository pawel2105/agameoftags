...
describe User do
  ...
  feature "admin" do
    it "returns true if ig_username is ADD_YOUR_USERNAME_HERE" do
      user = create(:user, ig_username: 'ADD_YOUR_USERNAME_HERE')
      expect(user.admin?).to be true
    end

    it "returns false if ig_username is not ADD_YOUR_USERNAME_HERE" do
  ...
  feature "find_or_create_from_uid" do
    ...
    it "returns updated existing user with matching UID" do
      ...
      Timecop.freeze(frozen_time) do
        @user = create(:user, uid: 'XYZ', ig_username: 'ADD_YOUR_USERNAME_HERE')
      end
